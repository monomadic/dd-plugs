...
impl SampleFile {
    ...
    pub fn sample_at(&self, offset: Playhead, freq: f64) -> i16 {
        let unity_freq = self.unity_pitch;
        let scale_factor = freq / unity_freq;

        let new_samplerate_ratio = self.sample_rate / 10000.0 * scale_factor;
        let new_pos = ((offset as f64) * new_samplerate_ratio) as usize;

        if self.samples.len() > new_pos {
//            info!("{}", self.samples[new_pos])
            self.samples[new_pos]
        } else { 0 }
    }
}
