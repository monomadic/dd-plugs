...
const INITIAL_SPREAD: f32 = 0.3;

/// Calculate the length in samples for a delay. Size ranges from 0.0 to 1.0.
fn delay(index: usize, mut size: f32, spread: f32) -> isize {
    const SIZE_OFFSET: f32 = 0.06;
    const SIZE_MULT: f32 = 10_000.0;
...
impl DimensionExpander {
    fn new(size: f32, dry_wet: f32) -> DimensionExpander {
        ...
        for i in 0..NUM_DELAYS {
            let samples = delay(i, size, INITIAL_SPREAD);
    ...
    fn resize(&mut self, n: f32) {
        ...
        for (i, buffer) in self.buffers.iter_mut().enumerate() {
            ...
            let old_delay = delay(i, old_size, self.spread);
            let new_delay = delay(i, n, self.spread);
